...
FastImputation <-
  function(
    ...
    if (missing(patterns)) {
      ...
      if (!methods::is(patterns, "FastImputationPatterns"))
        stop(
        ...
        if (length(cols_to_impute) == length(constrained_row)) {
          ...
          replacement_values <- t(t(patterns$FI_means[cols_to_impute])) +
            patterns$FI_covariance[cols_to_impute, known_cols, drop = FALSE] %*%
              solve(
                a = patterns$FI_covariance[known_cols, known_cols],
                b = t(constrained_row[known_cols]) -
                  t(t(patterns$FI_means[known_cols]))
              )
        }
    ...
    if (length(patterns$FI_cols_categorical) > 0) {
      for (i_col in 1:ncol(y_categorical)) {
        ...
        cat_indices <- apply(
          z[,
            (total_not_categorical + 1):(total_not_categorical +
              n_cols_this_var)
          ],
          1,
          which.max
