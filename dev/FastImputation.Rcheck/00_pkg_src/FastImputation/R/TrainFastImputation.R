...
function(
  ...
  if(!missing(idvars)) {
    if(is.numeric(idvars)) cols_to_ignore <- idvars
    else cols_to_ignore <- sapply(idvars, function(col_name) which(names(x) == col_name))
    y <- x[,-cols_to_ignore]
  ...
  if(!missing(categorical)) {
    if(is.numeric(categorical)) cols_categorical <- categorical
    else cols_categorical <- sapply(categorical, function(col_name) which(names(x) == col_name))
  }
