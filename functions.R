...
TrainFastImputation <- function(
  ...
  patterns <- list(FImeans=FastImputationMeans, FIcovariance=FastImputationCovariance)
...
FastImputation <- function(
  ...
  if( missing(patterns) ) {
    ...
    if( class(patterns) != "FastImputationPatterns" ) stop("'patterns' must be of class 'FastImputationPatterns'")
  }
  ...
  for(i.row in 1:n.rows) {
    ...
    replacement.values <- t( patterns$FImeans[cols.to.impute] + 
      patterns$FIcovariance[cols.to.impute,known.cols, drop=FALSE] %*% 
      solve(a=patterns$FIcovariance[known.cols,known.cols], 
        b=t(x[i.row,known.cols, drop=FALSE]) - t(t(patterns$FImeans[known.cols]))) )
    ...
    for(this.col in cols.to.constrain) {
      this.constraint <- constraints[[which(cols.to.constrain==this.col)]][[2]]
      x[i.row, this.col] <- ConstrainValues(x[i.row, this.col], constraints=this.constraint)
    }
