  ...
  function(
    ...
    if (missing(categorical)) {
      ...
      if (is.numeric(categorical)) {
        ...
        cols_categorical <- as.vector(sort(sapply(
          categorical,
          function(col_name) {
    ...
    if (0 == length(constraints)) {
      ...
      filled_constraints_in_x <- sapply(1:ncol(x), function(i_col) {
        is_each_constraint_for_this_col <- sapply(
          constraints,
          function(this_cons) {
        ...
        if (0 == sum(is_each_constraint_for_this_col)) {
          ...
          return(constraints[[max(which(is_each_constraint_for_this_col))]][[
            2
          ]])
        }
      })
    ...
    if (missing(idvars)) {
      ...
      if (is.numeric(idvars)) {
        ...
        cols_to_ignore <- as.vector(sort(sapply(
          idvars,
          function(col_name) which(names(x) == col_name)
        )))
      }
      y <- x[, -cols_to_ignore]
      ...
      if (length(cols_categorical) > 0) {
        for (cti in rev(sort(cols_to_ignore))) {
          cols_categorical[cols_categorical > cti] <- cols_categorical[
            cols_categorical > cti
          ] -
            1
        }
      }
    ...
    cols_in_y_bound_to_intervals <- which(sapply(
      filled_constraints_in_y,
      function(this_cons)
    ...
    for (this_col in cols_in_y_bound_to_intervals) {
      y[, this_col] <- NormalizeBoundedVariable(
        y[, this_col],
    ...
    if (length(cols_categorical) > 0) {
      ...
      if (total_not_categorical < 2) stop("Too few continuous variables.")
      if (total_one_hot_dummies > total_not_categorical)
        warning(
          "More categories in categorical variables than continuous variables. Performance will suffer."
        )
        ...
        for (i in 1:length(categories)) {
          for (j in 1:length(categories[[i]])) {
            z[, total_not_categorical + current_col_to_fill] <- ifelse(
              y_categorical[, i] == categories[[i]][j],
              1,
              -1
            )
    ...
    ) # for each index with categorical data, list the values observed (order here used)
